...
public:
	patcher(func_t patched_func)
	    : patched_func(patched_func), original_func(NULL)
	{
...
struct crit_section_lock {
	...
	crit_section_lock(CRITICAL_SECTION& crit)
	    : crit(crit)
	{
...
HRESULT find_output_device(std::wstring& output, string audioOutputName)
{
	...
	for (ULONG i = 0; i < count; i++) {
		...
		std::wstring audioOutpuNameWString = converter.from_bytes(audioOutputName);

		if (friendlyName.find(audioOutpuNameWString) == std::string::npos) {
			continue;
		}
