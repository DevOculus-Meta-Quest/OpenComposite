...
EVROverlayError BaseOverlay::SetOverlayTexture(VROverlayHandle_t ulOverlayHandle, const Texture_t *pTexture)
{
  ...
  if (overlay->compositor.get() == nullptr) {
    ...
    overlay->compositor = GetUnsafeBaseCompositor()->CreateCompositorAPI(&texture, size, false /*cubemapMode*/, true /*layerUse*/);
  }
  ...
  const auto srcSize = overlay->compositor->GetInputSrcSize();
  ...
  overlay->layerQuad.ColorTexture = overlay->compositor->GetOVRSwapChain();
